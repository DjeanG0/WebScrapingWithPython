...
# https://myacount.google.com/lesssecureapps
BASE_URl = 'http://remoteok.com/api/'
...
def send_email(send_from, send_to, subject, text, files=None):
    ...
    smtp.starttls()
    smtp.login(send_from, 'mqvrvblosmvfrgtr')
    smtp.sendmail(send_from, send_to, msg.as_string())
    smtp.close()
if __name__ == "__main__":
    json = get_job_postings()[1:]
    output_job_to_xls(json)
    send_email('jecamureyes@gmail.com', ['jean.munoz@uptc.edu.co'], 'Jobs Postings', 
               'Opportunities to work on', files=['remote_jobs.xls'])
    print('Email send')